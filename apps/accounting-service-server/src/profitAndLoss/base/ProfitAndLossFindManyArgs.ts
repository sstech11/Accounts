/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProfitAndLossWhereInput } from "./ProfitAndLossWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ProfitAndLossOrderByInput } from "./ProfitAndLossOrderByInput";

@ArgsType()
class ProfitAndLossFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ProfitAndLossWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ProfitAndLossWhereInput, { nullable: true })
  @Type(() => ProfitAndLossWhereInput)
  where?: ProfitAndLossWhereInput;

  @ApiProperty({
    required: false,
    type: [ProfitAndLossOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ProfitAndLossOrderByInput], { nullable: true })
  @Type(() => ProfitAndLossOrderByInput)
  orderBy?: Array<ProfitAndLossOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ProfitAndLossFindManyArgs as ProfitAndLossFindManyArgs };
